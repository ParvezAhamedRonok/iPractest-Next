...
import "../ListeningAllStyles/ListeningFooter.css";
import ListeningCounterDown from "../../../../Reading-Module/Reading-All-Tests/[...AllReadingTest]/Important/CounterDown";
...
import SubmitListening from './SubmitListening';
...
function Footer({ UserData, AnswersData, setPassComment, setWaveTit, testName }) {
  ...
  console.log(ListeningShowAnswer);
  // for submit all users data by post request-- ----------->
  const handleListeningUsersData = (x) => {
    x.preventDefault();
    setListeningShowAnswer(testName)
  ...
  return (
    ...
    <section className='L-FooterSection'>
      <div className="div_1">
        ...
        {/* Usersdata post by Submit & make Review with the Review buttons */}
        <div className="right_btn div_colum">
          ...
          <i
            className="right_icon icons cursor-pointer"
            onClick={() => {
      ...
      {/* for showing the Preview & Submit Sections when it will be true by it's state*/}
      {
        togglePreview && <ListeningPreview
          UserData={UserData}
          handleReviewPage={handleReviewPage} />
      }
      {
        toggleSubmit && <SubmitListening
          UserData={UserData}
          handleListeningUsersData={handleListeningUsersData}
