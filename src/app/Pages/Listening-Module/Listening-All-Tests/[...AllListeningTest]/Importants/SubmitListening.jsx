...
import "../ListeningAllStyles/submitListening.css";
import Axios from 'axios';
import { useRouter } from 'next/navigation';
import { MdOutlineCancel } from "react-icons/md";
import { LISTENING_GET_SPECIFIC_USER_READING_LAST_DATA, LISTENING_POST_FOR_SET_SCORES } from "../../../../../../assets/URL's/AllUrl"
// end of the importing..



function SubmitListening({ handleListeningUsersData, UserData, AnswersData, setPassComment, setWaveTit, testName }) {
     const history = useRouter();
     //all localstorage data storing by the help of those thates..
     const [ModulesListening, setModulesListening] = useState("");
     const [userCountry, setuserCountry] = useState("");
     const [useLoginToken, setUserLoginToken] = useState("");
     const [userLoginEmail, setUserLoginEmail] = useState("");
     const [userLoginName, setUseLoginName] = useState("")
     ...
     useEffect(() => {
          ...
          setUseLoginName(localStorage.getItem("userName"))

     }, [])

     //for showing the user comments icon
     useEffect(() => {
          if (count > 0) {
               setTimeout(() => {
                    setPassComment(true);
     ...
     const PostReadingScors = (x) => {
          ...
          Axios({
               method: "post",
               url: LISTENING_POST_FOR_SET_SCORES,
               headers: {
                    "Authorization": `Bearer ${useLoginToken}`,
     ...
     //not need because already all data comming from database directly------------
     const getUserLatestDataFromDatabaseSave = (localItem) => {
          setTimeout(() => {
               Axios({
                    method: "get",
                    // url: 'http://localhost:8080/api/user/get-SpecificUser-Listening-Data-last-One/' + localStorage.getItem('userEmail') + "+" + testName,
                    url: LISTENING_GET_SPECIFIC_USER_READING_LAST_DATA + userLoginEmail + "+" + testName,
     ...
     useEffect(() => {
          if (ReadingRate > 0 && userLoginName && userLoginEmail) {
               PostReadingScors();
               if (ModulesListening == "1") {
                    localStorage.setItem("L_OkmoduleNo1", "Test1_Ok");
                    localStorage.setItem("L_ScorModuleNo1", ReadingRate.toFixed(1));
                    // getUserLatestDataFromDatabaseSave("L_ScorModuleNo1")
               };
               if (ModulesListening == "2") {
                    localStorage.setItem("L_OkmoduleNo2", "Test2_Ok");
                    localStorage.setItem("L_ScorModuleNo2", ReadingRate.toFixed(1));
                    // getUserLatestDataFromDatabaseSave("L_ScorModuleNo2")
               }
               if (ModulesListening == "3") {
                    localStorage.setItem("L_OkmoduleNo3", "Test3_Ok");
                    localStorage.setItem("L_ScorModuleNo3", ReadingRate.toFixed(1));
                    // getUserLatestDataFromDatabaseSave("L_ScorModuleNo3")
               }
               if (ModulesListening == "4") {
                    localStorage.setItem("L_OkmoduleNo4", "Test4_Ok");
                    localStorage.setItem("L_ScorModuleNo4", ReadingRate.toFixed(1));
                    // getUserLatestDataFromDatabaseSave("L_ScorModuleNo4")
               }
               if (ModulesListening == "5") {
                    localStorage.setItem("L_OkmoduleNo5", "Test5_Ok");
                    localStorage.setItem("L_ScorModuleNo5", ReadingRate.toFixed(1));
                    // getUserLatestDataFromDatabaseSave("L_ScorModuleNo5")
               }
               if (ModulesListening == "6") {
                    localStorage.setItem("L_OkmoduleNo6", "Test6_Ok");
                    localStorage.setItem("L_ScorModuleNo6", ReadingRate.toFixed(1));
                    // getUserLatestDataFromDatabaseSave("L_ScorModuleNo6")
               }
               if (ModulesListening == "7") {
                    localStorage.setItem("L_OkmoduleNo7", "Test7_Ok");
                    localStorage.setItem("L_ScorModuleNo7", ReadingRate.toFixed(1));
                    // getUserLatestDataFromDatabaseSave("L_ScorModuleNo7")
               }
               if (ModulesListening == "8") {
                    localStorage.setItem("L_OkmoduleNo8", "Test8_Ok");
                    localStorage.setItem("L_ScorModuleNo8", ReadingRate.toFixed(1));
                    // getUserLatestDataFromDatabaseSave("L_ScorModuleNo8")
               }
               if (ModulesListening == "9") {
                    localStorage.setItem("L_OkmoduleNo9", "Test9_Ok");
                    localStorage.setItem("L_ScorModuleNo9", ReadingRate.toFixed(1));
                    // getUserLatestDataFromDatabaseSave("L_ScorModuleNo9")
               }
               if (ModulesListening == "10") {
                    localStorage.setItem("L_OkmoduleNo10", "Test10_Ok");
                    localStorage.setItem("L_ScorModuleNo10", ReadingRate.toFixed(1));
                    // getUserLatestDataFromDatabaseSave("L_ScorModuleNo10")
               }
               if (ModulesListening == "11") {
                    localStorage.setItem("L_OkmoduleNo11", "Test11_Ok");
                    localStorage.setItem("L_ScorModuleNo11", ReadingRate.toFixed(1));
                    // getUserLatestDataFromDatabaseSave("L_ScorModuleNo11")
               }
               if (ModulesListening == "12") {
                    localStorage.setItem("L_OkmoduleNo12", "Test12_Ok");
                    localStorage.setItem("L_ScorModuleNo12", ReadingRate.toFixed(1));
                    // getUserLatestDataFromDatabaseSave("L_ScorModuleNo12")
               }
               // for Cambridge showing Answers


               if (ModulesListening == "cambridge-1") {
                    localStorage.setItem("L_Okcambridge1", "Okcambridge_1");
                    localStorage.setItem("L_ScorOkcambridge1", ReadingRate.toFixed(1));
                    // getUserLatestDataFromDatabaseSave("L_ScorOkcambridge1")
               }
               if (ModulesListening == "cambridge-2") {
                    localStorage.setItem("L_Okcambridge2", "Okcambridge_2");
                    localStorage.setItem("L_ScorOkcambridge2", ReadingRate.toFixed(1));
                    // getUserLatestDataFromDatabaseSave("L_ScorOkcambridge2")
               }
               if (ModulesListening == "cambridge-3") {
                    localStorage.setItem("L_Okcambridge3", "Okcambridge_3");
                    localStorage.setItem("L_ScorOkcambridge3", ReadingRate.toFixed(1));
                    // getUserLatestDataFromDatabaseSave("L_ScorOkcambridge3")
               }
               if (ModulesListening == "cambridge-4") {
                    localStorage.setItem("L_Okcambridge4", "Okcambridge_4");
                    localStorage.setItem("L_ScorOkcambridge4", ReadingRate.toFixed(1));
                    // getUserLatestDataFromDatabaseSave("L_ScorOkcambridge4")
               }
               if (ModulesListening == "cambridge-5") {
                    localStorage.setItem("L_Okcambridge5", "Okcambridge_5");
                    localStorage.setItem("L_ScorOkcambridge5", ReadingRate.toFixed(1));
                    // getUserLatestDataFromDatabaseSave("L_ScorOkcambridge5")
               }
               if (ModulesListening == "cambridge-6") {
                    localStorage.setItem("L_Okcambridge6", "Okcambridge_6");
                    localStorage.setItem("L_ScorOkcambridge6", ReadingRate.toFixed(1));
                    // getUserLatestDataFromDatabaseSave("L_ScorOkcambridge6")
               }
               if (ModulesListening == "cambridge-7") {
                    localStorage.setItem("L_Okcambridge7", "Okcambridge_7");
                    localStorage.setItem("L_ScorOkcambridge7", ReadingRate.toFixed(1));
                    // getUserLatestDataFromDatabaseSave("L_ScorOkcambridge7")
               }
               if (ModulesListening == "cambridge-8") {
                    localStorage.setItem("L_Okcambridge8", "Okcambridge_8");
                    localStorage.setItem("L_ScorOkcambridge8", ReadingRate.toFixed(1));
                    // getUserLatestDataFromDatabaseSave("L_ScorOkcambridge8")
               }
               if (ModulesListening == "cambridge-9") {
                    localStorage.setItem("L_Okcambridge9", "Okcambridge_9");
                    localStorage.setItem("L_ScorOkcambridge9", ReadingRate.toFixed(1));
                    // getUserLatestDataFromDatabaseSave("L_ScorOkcambridge9")
               }
               if (ModulesListening == "cambridge-10") {
                    localStorage.setItem("L_Okcambridge10", "Okcambridge_10");
                    localStorage.setItem("L_ScorOkcambridge10", ReadingRate.toFixed(1));
                    // getUserLatestDataFromDatabaseSave("L_ScorOkcambridge10")
               }
               if (ModulesListening == "cambridge-11") {
                    localStorage.setItem("L_Okcambridge11", "Okcambridge_11");
                    localStorage.setItem("L_ScorOkcambridge11", ReadingRate.toFixed(1));
                    // getUserLatestDataFromDatabaseSave("L_ScorOkcambridge11")
               }
               if (ModulesListening == "cambridge-12") {
                    localStorage.setItem("L_Okcambridge12", "Okcambridge_12");
                    localStorage.setItem("L_ScorOkcambridge12", ReadingRate.toFixed(1));
                    // getUserLatestDataFromDatabaseSave("L_ScorOkcambridge12")
               }
          ...
          >
               <div className="grid w-[100vw] h-[100%] justify-center align-middle relative">
                    <div className='absolute top-0 left-0 right-0 bottom-0 bg-transparent z-[1000]'
                         onClick={handleListeningUsersData}
                    ></div>
                         ...
                         <div className='w-full flex justify-between'>
                              ...
                              <div className='mr-3'>
                                   <button
                                        ...
                                        onClick={handleListeningUsersData}>  <MdOutlineCancel />
                                   </button>
                              </div>
