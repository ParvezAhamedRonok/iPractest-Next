...
import { SpeakingArray } from '../../data/LineChartData';
...
const WeekSpeakingChart = () => {
  ...
  const WeekSpeakingLinePrimaryYAxis = {
    labelFormat: '{value}',
    rangePadding: 'None',
    minimum: 0,
    maximum: 9,
  ...
  // let main = SpeakingArray.filter((w) => w.x == '2023/12/07');
  let pushDay1 = SpeakingArray.filter((w) => w.x == thisDay);
  let pushDay2 = SpeakingArray.filter((w) => w.x == lastDay1);
  let pushDay3 = SpeakingArray.filter((w) => w.x == lastDay2);
  let pushDay4 = SpeakingArray.filter((w) => w.x == lastDay3);
  let pushDay5 = SpeakingArray.filter((w) => w.x == lastDay4);
  let pushDay6 = SpeakingArray.filter((w) => w.x == lastDay5);
  let pushDay7 = SpeakingArray.filter((w) => w.x == lastDay6);
  pushDay7.map((x) => {
    let mainX = {
  ...
  return (
    <div className='control-pane'>
      ...
      <div className='control-section'>
        <ChartComponent id='charts-1' style={{ textAlign: "center",borderRadius:"25px",border:"2px solid blue" }}
          primaryXAxis={{ valueType: 'Category', minorGridLines: { width: 0 }, majorGridLines: { width: 0 } }}
          primaryYAxis={WeekSpeakingLinePrimaryYAxis}
          // width={Browser.isDevice ? '100%' : '35%'}
          // width={'30%'}
          height={'55%'}
          chartArea={{ border: { width: 0 } }}
          legendSettings={{ visible: false }}
          title='Speaking weekly performance'
          tooltip={{ enable: true }}
        // background={"skyblue"}
        >
          ...
          <SeriesCollectionDirective>
            <SeriesDirective dataSource={data1} xName='x' yName='y' width={2} name='Speaking' type='Column' marker={{ visible: true, width: 7, height: 7 }} />
          </SeriesCollectionDirective>
        </ChartComponent>
      </div>
    </div>
...
export default WeekSpeakingChart;
