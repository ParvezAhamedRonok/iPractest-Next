...
import { ReadingArray } from '../../data/LineChartData';
...
const WeekReadingChart = () => {
  ...
  const WeekReadingLinePrimaryYAxis = {
    labelFormat: '{value}',
    rangePadding: 'None',
    minimum: 0,
    maximum: 9,
  ...
  // let main = ReadingArray.filter((w) => w.x == '2023/12/07');
  let pushDay1 = ReadingArray.filter((w) => w.x == thisDay);
  let pushDay2 = ReadingArray.filter((w) => w.x == lastDay1);
  let pushDay3 = ReadingArray.filter((w) => w.x == lastDay2);
  let pushDay4 = ReadingArray.filter((w) => w.x == lastDay3);
  let pushDay5 = ReadingArray.filter((w) => w.x == lastDay4);
  let pushDay6 = ReadingArray.filter((w) => w.x == lastDay5);
  let pushDay7 = ReadingArray.filter((w) => w.x == lastDay6);
  ...
  return (
    <div className='control-pane'>
      ...
      <div className='control-section'>
        <ChartComponent id='charts-2' style={{ textAlign: "center",borderRadius:"25px",border:"2px solid blue" }}
          primaryXAxis={{ valueType: 'Category', minorGridLines: { width: 0 }, majorGridLines: { width: 0 } }}
          primaryYAxis={WeekReadingLinePrimaryYAxis}
          // width={Browser.isDevice ? '100%' : '35%'}
          // width={'30%'}
          height={'55%'}
          chartArea={{ border: { width: 0 } }}
          legendSettings={{ visible: false }}
          title='Reading weekly performance'
          tooltip={{ enable: true }}
        // background={"blue"}
        >
          ...
          <SeriesCollectionDirective>
            <SeriesDirective dataSource={data1} xName='x' yName='y' width={2} name='Reading' type='Column' marker={{ visible: true, width: 7, height: 7 }} />
          </SeriesCollectionDirective>
        </ChartComponent>
      </div>
    </div>
...
export default WeekReadingChart;
